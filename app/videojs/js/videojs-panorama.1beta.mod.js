...
var BaseCanvas = function (_Component) {
    ...
    function BaseCanvas(player, options, renderElement) {
     ...
     //MOD: bind create a new function at each call, so add & remove Listener haven't same callback function
     //     so removeListener don't recognize addListener and event is not dispose. Have to define a global
     //     var as this.handleResize associated to handleResize.bind(_this) to be sure callback are the same.
        _this.handleResize            = _this.handleResize.bind(_this);
        _this.handleMobileOrientation = _this.handleMobileOrientation.bind(_this);
      ...
      //MOD
        ...
        value: function attachControlEvents() {
            ...
            if (this.options.resizable) {
             // MOD     this.handleResize is now a global var for this.handleResize.bind(this), otherwise removeEventListener don't work
            ...
            if (this.options.autoMobileOrientation) {
             ...
             // window.addEventListener('devicemotion', this.handleMobileOrientation.bind(this));
                window.addEventListener('devicemotion', this.handleMobileOrientation);
            }
        ...
        value: function detachControlEvents() {
            ...
            this.off('resize', this.handleResize.bind(this));
            if (this.options.scrollable) {
            ...
            if (this.options.autoMobileOrientation) {
             ...
             // window.removeEventListener('devicemotion', this.handleMobileOrientation.bind(this));
                window.removeEventListener('devicemotion', this.handleMobileOrientation);
            }
            if (this.options.KeyboardControl) {
            // MOD
        ...
        value: function handleMobileOrientation(event) {
            if (typeof event.rotationRate !== "undefined") {
                ...
                var x = event.rotationRate.alpha;    //MOD  * Math.PI / 180
                var y = event.rotationRate.beta;     //MOD  * Math.PI / 180
...
var Marker = function (_Component) {
    ...
    function Marker(player, options) {
        ...
        if (typeof elem === "string") {
          // MOD:
             if (elem = 'link') {
                  el = document.createElement('a');   // el.title = elem;
                  el.className = 'vjs-marker ' + options.className;
                  var js = options.onClick;
                  el.href = 'javascript:' + js;
             }else{
     ...
     // el.className = 'vjs-marker ';
      ...
      //MOD:
            _this.isLeft = options.isLeft;
            _this.is3D  = options.is3D;
            _this.is180  = options.is180;
        var is3D180_LON = _this.is180 ? 0.5 : 1;       //for 3D180, only half of longitude is used, so Marker option lon has to be reduced by 2: options.location.lon -> options.location.lon * 0.5
        var longitude = (_this.is180 ? options.location.lon180 : options.location.lon) * is3D180_LON;   //lon180: needed to avoid marker too close as longitude are shrink 360 -> 180
        var is3DRight = this.is3D && !this.isLeft;    //if 3D, Right view is projected in a sphere (radius 500) placed at (1000,0,0) like the camera definition. 
        var Sphere_Origin_X =  is3DRight ? 1000 : 0;   //So Marker vector must be also shifted to this new origin: add 1000 to x        
      //MOD
      
        var phi = _three2.default.Math.degToRad((90 - options.location.lat));   
      //MOD:       //definition of Marker Vector in the projection sphere
      ...
      //_this._position = new _three2.default.Vector3( options.radius * Math.sin(phi) * Math.cos(theta), options.radius * Math.cos(phi), options.radius * Math.sin(phi) * Math.sin(theta));
        var theta = _three2.default.Math.degToRad(longitude);
        _this._position = new _three2.default.Vector3( Sphere_Origin_X + options.radius * Math.sin(phi) * Math.cos(theta), options.radius * Math.cos(phi), options.radius * Math.sin(phi) * Math.sin(theta));
      //MOD                                          // 1000 + 500 * sin phi etc.
    ...
    _createClass(Marker, [{
        ...
        value: function render(canvas, camera) {
         ...
         // MOD:   
            var camera_origin = camera.position.clone();                        //Projection for Left/Right view is done on 2 threejs sphere (radius 500). Left sphere is at 0,0,0 but right is shifted of 1000 in x at (1000,0,0) and same for cameraR. When calculate angle for right view, need to take back vector to right sphere origin, otherwise vector coordonates are done from threejs origin (0,0,0) and there is a 1000 factor in x that mess.
            var marker_position = this._position.clone().sub(camera_origin);    // objects are passed by ref in js, clone object to avoid change original
            var target_position = camera.target.clone().sub(camera_origin);     // Needed for Rigth view and change nothing for Left View
            var angle = marker_position.angleTo(target_position);               //Equivalent to x -> x - 1000
         //MOD
            if (angle > Math.PI * 0.4) {
                this.addClass("vjs-marker--backside");
            } else {
            //MOD:
                this.options.el.style.opacity  = Math.max(0.2, 1 - 6 * angle / Math.PI);
                var isFocus = $(this.options.el).hasClass('vjs-marker-focus');
                 if (angle < 5 * Math.PI / 180) {      // target is around 4° of marker
                      if(!isFocus){
                          this.addClass("vjs-marker-focus");
                          if(this.isLeft) this.Wait = setTimeout(
                              function(me){me.options.el.click();}, 2000, this);};
                 } else {
                      if(isFocus) {
                          if(this.isLeft) clearTimeout(this.Wait); 
                          this.removeClass("vjs-marker-focus");};
                 };
            //MOD    
                this.removeClass("vjs-marker--backside");
                var vector = this._position.clone().project(camera);   //Point3DToScreen2D
...
var MarkerContainer = function (_Component) {
    ...
    function MarkerContainer(player, options) {
        ...
        if (_this.options.VREnable) {
                ...
                var videoType = _this.options.videoType;                          //MOD
                var is3D      = (videoType.toUpperCase().indexOf('3D') >= 0);     //MOD
                var is180     = (videoType.toUpperCase().indexOf('180') >= 0);    //MOD
            ...
            (function () {
                ...
                //  camera:  _this._canvas._camera                                   //MOD
                    camera: is3D ? _this._canvas._cameraL : _this._canvas._camera,   //MOD
                
                    videoType: videoType,                                            //MOD
                    is3D: is3D,                                                      //MOD
                    is180: is180,                                                    //MOD
                    isLeft: true                                                     //MOD
                });
                ...
                //  camera:  _this._canvas._camera                                   //MOD
                    camera: is3D ? _this._canvas._cameraR : _this._canvas._camera,   //MOD
                  //  camera: _this._canvas._cameraR ,
                    videoType: videoType,                                            //MOD
                    is3D: is3D,                                                      //MOD
                    is180: is180,                                                    //MOD
                    isLeft: false                                                   //MOD                   
            ...
            //MOD:  leftMarkerGroup.camera = _this._canvas._camera;
                    leftMarkerGroup.camera = is3D ? _this._canvas._cameraL : _this._canvas._camera;  
...
var MarkerGroup = function (_Component) {
    ...
    function MarkerGroup(player, options) {
        ...
        _this._videoType = options.videoType;                //MOD
        _this._is3D= options.is3D;                           //MOD
        _this._is180 = options.is180;                        //MOD
        _this._isLeft = options.isLeft;                      //MOD

        _this.options.markers.forEach(function (markSetting) {
        
              var view       = markSetting.view;                                           //MOD   //flag: show marker on VR left/view side
              var viewL      = (view.toUpperCase().indexOf('L') >= 0);                     //MOD
              var viewR      = (view.toUpperCase().indexOf('R') >= 0);                     //MOD
              var showMarker = ( (_this._isLeft && viewL) || (!_this._isLeft && viewR) );  //MOD
              
              var is180Back  = options.is180 && markSetting.side == 'back'                 //MOD   //if VR180, don't add back marker, as there is no back

              if( showMarker && !is180Back )                                                 //MOD
    ...
    _createClass(MarkerGroup, [{
        ...
        value: function addMarker(markSetting) {
            ...
            markSetting.videoType = this._videoType;   //MOD
            markSetting.is3D = this._is3D  ;           //MOD 
            markSetting.is180 = this._is180  ;         //MOD              
            markSetting.isLeft = this._isLeft;         //MOD  
...
var ThreeDVideo = function (_BaseCanvas) {
    ...
    //MOD:
        key: 'dispose',
        value: function dispose() {                       //if TwoDVideo has also a dispose method, BaseCanvas.dispose(Parent) is no more called, although it is more important, so need to call it with _BaseCanvas.prototype.dispose.call
            _BaseCanvas.prototype.dispose.call(this)    // call BaseCanvas.dispose (Parent), needed to reset other objects and el().
         // this._scene.remove(this.mesh);
        
            this._cameraL = null;
            this._cameraR = null;
         // this._camera.target = null;    //undefined
            this._scene = null;
       }
    //MOD
    }]);
...
//MOD Start: Add module 220 for VR 360 3D Left-Right
...
var VR3603DLR = function (_ThreeDVideo) {
    _inherits(VR3603DLR, _ThreeDVideo);

    function VR3603DLR(player, options, renderElement) {
        _classCallCheck(this, VR3603DLR);

        var _this = _possibleConstructorReturn(this, (VR3603DLR.__proto__ || Object.getPrototypeOf(VR3603DLR)).call(this, player, options, renderElement));
        ...
        _this._meshL = new THREE.Mesh(geometryL, new THREE.MeshBasicMaterial({ map: _this._texture }));

        _this._meshR = new THREE.Mesh(geometryR, new THREE.MeshBasicMaterial({ map: _this._texture }));
        _this._meshR.position.set(1000, 0, 0);
...
exports.default = VR3603DLR;

}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})

},{"./ThreeDVideo":19}],
//MOD End: Add module 220 for VR 360 3D Left-Right
...
//MOD Start: Add module 220 for VR 360 3D Left-Right
var _VR3603DLR = require('./Components/VR3603DLR');

var _VR3603DLR2 = _interopRequireDefault(_VR3603DLR);
//MOD End: Add module 220 for VR 360 3D Left-Right
...
//MOD: Add module 220 for VR 360 3D Left-Right
var videoTypes = ["equirectangular", "fisheye", "dual_fisheye", "VR1803D", "VR3603D", "VR3603DLR"]; 
...
var defaults = exports.defaults = {
    ...
    mobileVibrationValue: (0, _utils.isIos)() ? 0.022 : 1,     //MOD: 0.022 : 1  plus nécessaire avec MOD: event.rotationRate.alpha * Math.PI / 180;
...
//MOD: Add module 220 for VR 360 3D Left-Right                  
                case "VR3603DLR":
                    VideoClass = _VR3603DLR2.default;
                    break;                    
                case "VR1803D":
    ...
    _createClass(Panorama, [{
        ...
        //MOD:
            // this.player.off('VRModeOn');      //Remove event
            // this.player.off('VRModeOff');
            ...
            // this.player.removeComponent("VideoCanvas");   //removeComponent   ( it calls also BaseCanvas.dispose() )
            // this.player.removeComponent("HelperCanvas");
            // this.player.removeComponent('Thumbnail'); 
            // this.player.removeComponent('ThumbnailCanvas'); 
            // this.player.removeComponent("markerContainer");
            // this.player.removeComponent('Notice'); 
            // this.player.removeComponent('VRButton');
            // this.player.getVideoEl().remove();
         //MOD
        }
    }, {
        key: 'attachEvents',
            ...
            var handlePlay = function handlePlay() {
                ...
                if (_this2.options.Markers && Array.isArray(_this2.options.Markers)) {
                    var markerContainer = new _MarkerContainer2.default(_this2.player, {
                        ...
                        videoType: _this2.options.videoType   //MOD: give info on videoType to markerContainer to choose good camera
                    });
